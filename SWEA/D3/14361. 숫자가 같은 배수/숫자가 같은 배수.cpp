...
int main(int argc, char** argv)
{
	...
	*/
	for(test_case = 1; test_case <= T; ++test_case)
	{
        ...
        vector<int> Biggest = v;
        vector<int> Smallest = v;
        sort(Biggest.begin(),Biggest.end());
        sort(Smallest.begin(),Smallest.end());
        reverse(Biggest.begin(),Biggest.end());
        
        int biggest = vector_to_int(Biggest);
        int smallest = vector_to_int(Smallest);
        
        int ans = 0;
        if(biggest > n){
            for(int i=2; i*n<=biggest ; i++){
                vector<int> tmp = int_to_vector(i*n);
                
                sort(tmp.begin(),tmp.end());
                
                int x = vector_to_int(tmp);
                if(smallest == x){
                    ans = 1;
                    break;
                }
